...
open class PopTip: UIView {
    ...
    internal func setupVertically() -> (CGRect, CGPoint) {
        ...
        if constrainInContainerView && (x + frame.width > containerView.bounds.width) {
            x = containerView.bounds.width - frame.width - edgeMargin < 0 ? 0 : containerView.bounds.width - frame.width - edgeMargin
        }
        ...
        frame.size = CGSize(width: frame.width, height: frame.height + borderWidth * 2)
    ...
    public func calculateFutureDirection(preferredDirection: PopTipDirection,in containerView: UIView, from: CGRect) -> PopTipDirection {
        var result: PopTipDirection = .none
        backgroundColor = .clear
        
        // Decide the direction if not specified
        if preferredDirection.isAuto {
            var spaces: [PopTipDirection: CGFloat] = [:]
            
            if preferredDirection == .autoHorizontal || preferredDirection == .auto {
                spaces[.left] = from.minX - containerView.frame.minX
                spaces[.right] = containerView.frame.maxX - from.maxX
            }
            
            if preferredDirection == .autoVertical || preferredDirection == .auto {
                spaces[.up] = from.minY - containerView.frame.minY
                spaces[.down] = containerView.frame.maxY - from.maxY
            }
            
            result = spaces.sorted(by: { $0.1 > $1.1 }).first!.key
        }
